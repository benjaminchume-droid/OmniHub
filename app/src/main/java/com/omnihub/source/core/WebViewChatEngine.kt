 ...
 * WebRelay 1.0.9.9
 *
 * - PageMap hot path: reuse learned composer/send (ms after first success)
 * - Adaptive timeout: idle 55s, but each partial extends; absolute max 3 min for long gens
 * - Last-turn + chrome strip for accuracy
 */
object WebViewChatEngine {
    ...
    /** Quiet with no tokens → fail */
    private const val IDLE_TIMEOUT_MS = 55_000L
    /** Hard ceiling even while streaming (long novel chapters) */
    private const val ABS_MAX_MS = 180_000L
    /** Extend deadline this much after each partial */
    private const val PARTIAL_EXTEND_MS = 45_000L
    ...
    fun cleanReply(raw: String, userMessage: String): String? {
        ...
        if (um.isNotEmpty()) {
            if (text.length in 1..um.length && um.endsWith(text, ignoreCase = true)) return null
            if (text.startsWith(um, ignoreCase = true)) text = text.substring(um.length).trim()
        }
        ...
        val chromeHits = listOf(
            ...
            "check important info", "conversation with", "meet your ai agents",
            "ask a follow-up", "code faster with", "automate more with"
        ).count { it in lower }
        // Long real answers can mention products once — only reject short chrome blobs
        if (chromeHits >= 2 && text.length < 400) return null
        if (chromeHits >= 4 && text.length < 800) return null
        return text
    }
    ...
    private suspend fun sendExclusive(
        ...
        val mapHot = pageMap.isUsable()
        val target = siteUrl.ifBlank {
            ProviderCatalog.urlFor(providerId) ?: "https://chatgpt.com/"
        }

        Log.i(TAG, "[$txn] START provider=$providerId mapHot=$mapHot queueWaitMs=$queueWaitMs")
        ...
        main.post {
            ...
            fun scheduleTimeout(delayMs: Long) {
                ...
                val remainingAbs = (ABS_MAX_MS - elapsed).coerceAtLeast(0L)
                val delay = delayMs.coerceAtMost(remainingAbs)
                if (delay <= 0L) {
                    main.post(timeoutRunnable)
                } else {
                    main.postDelayed(timeoutRunnable, delay)
                }
            }
            ...
            timeoutRunnable = Runnable {
                // Prefer last streamed partial if clean; else page salvage
                val fromStream = cleanReply(lastPartialText, userMessage)
                if (fromStream != null && fromStream.length > 20) {
                    finish(fromStream, true)
                    return@Runnable
                }
                web.evaluateJavascript(
                    """
                    ...
                    if (cleaned != null && cleaned.length > 12) {
                        finish(cleaned, true)
                    } else if (lastPartialText.length > 40) {
                        // Deliver partial even if chrome filter was aggressive
                        val soft = ReplySanitizer.strip(lastPartialText)
                        finish(soft.ifBlank { lastPartialText }, soft.length > 8)
                    } else {
                        finish("REPLY_TIMEOUT txn $txn — no clean reply. Stay on signed-in chat and retry.", false)
                    }
                }
            }
            ...
            val bridge = object {
                    ...
                    main.post {
                        ...
                        when {
                            ...
                            t.isBlank() -> finish("Empty reply.", false)
                                ...
                                val cleaned = cleanReply(t, userMessage)
                                    ?: ReplySanitizer.strip(t).takeIf { it.length > 8 }
                                    ?: t
                                finish(cleaned, cleaned.length > 1)
                            }
                        }
                    }
                }
                ...
                fun onPartial(text: String) {
                    main.post {
                        ...
                        lastPartialText = raw
                        // Adaptive: keep waiting while the model is still writing
                        scheduleTimeout(PARTIAL_EXTEND_MS)
                        val cleaned = cleanReply(raw, userMessage)
                        if (cleaned != null && cleaned.length >= 3) {
                            ...
                            // Stream longer raw for long chapters even before full clean
                            if (firstTokenAt == 0L) firstTokenAt = System.currentTimeMillis()
                            val soft = ReplySanitizer.strip(raw)
                            if (soft.length >= 8) onPartial(soft)
                        }
                    }
                }
            ...
            fun waitComposer(attempt: Int) {
                if (finished || injected) return
                // Learned map: try inject immediately (ms path)
                if (mapHot && attempt == 0) {
                    inject()
                    return
                }
                ...
                web.evaluateJavascript(
                    ...
                    else main.postDelayed({ waitComposer(attempt + 1) }, 60)
                }
            }
            ...
            if (session.ready && sameHost && session.lastUrl.isNotBlank()) {
                Log.i(TAG, "[$txn] HOT_SESSION mapHot=$mapHot")
                waitComposer(0)
            } else {
                session.ready = false
    ...
    private fun buildEngineScript(
        ...
        val mapHotJs = if (mapHot) "true" else "false"
        return """
(function(){
  if (window.__omniObs) { try { window.__omniObs.disconnect(); } catch(e){} window.__omniObs=null; }
  ...
  function findInput(){
    // Learned selector first when map is hot
    if(MAP_IN){
      try{
        var le=document.querySelector(MAP_IN);
        if(le&&visible(le)&&(MAP_HOT||scoreInput(le)>=5)) return le;
      }catch(e){}
    }
    var preferred=[
  ...
  function fingerprint(){
    ...
    return raw.split('\n').map(function(l){return l.trim();}).filter(function(l){
      if(!l) return false;
      if(/^(chat|agent|new chat|sign in|log in|terms of service|privacy policy|menu|settings|home|share|copy|deep think|max|flash|ask a follow-up|zcode|autoclaw)$/i.test(l)) return false;
      return true;
    }).join('\n');
  }
  ...
  function deltaAfterBaseline(beforeFp){
    ...
    if(snip){
      ...
      if(uIdx>=0) d=after.slice(uIdx+snip.length).trim();
    }
    ...
    d=d.split('\n').map(function(l){return l.trim();}).filter(function(l){
      if(!l) return false;
      if(/^(you said|gemini said|chatgpt said|claude said|deep think|max|flash|conversation with|ask a follow-up|ai ppt|zcode|autoclaw)/i.test(l)) return false;
      if(/can make mistakes|check important info|generated by ai|meet your ai agents|code faster with|automate more with/i.test(l)) return false;
      if(MSG && l.toLowerCase()===MSG.toLowerCase()) return false;
      if(/^\d{1,2}:\d{2}\s*(AM|PM)?$/i.test(l)) return false;
      return true;
    }).join('\n').trim();
    return d;
  }
  ...
  state(MAP_HOT ? 'COMPOSER_MAP_HIT' : 'COMPOSER_RESOLVED');
  ...
  function verifyInput(attempt, cont){
    ...
    if(attempt===0){
      fill(input, MSG);
      setTimeout(function(){ verifyInput(1, cont); }, 40);
      return;
    }
    cont(false);
  }
  ...
  verifyInput(0, function(okInput){
    ...
    confirmSend(function(sent){
      ...
      function tick(){
        ...
        // Short replies: complete fast. Long replies: need more quiet after growth stops.
        var needQuiet = d.length > 2000 ? 4 : 2;
        if(grew && quiet>=needQuiet && !gen && d.length>=3){
          state('COMPLETED');
      ...
      // JS-side ceiling aligned with adaptive Kotlin timeout (3 min)
      window.__omniTimer=setInterval(function(){
        if(tick()) return;
        if(Date.now()-tStart>175000){
